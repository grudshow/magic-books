...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		buttonPopup.classList.add('_disabled');
		questionButton.classList.add('_disabled');
	...
	if (unlock) {
		if (!item) {
			for (let index = 0; index < popups.length; index++) {
				...
				buttonPopup.classList.remove('_disabled');
				questionButton.classList.remove('_disabled');
			}
		} else {
...
//Tippy
const buttons = document.querySelectorAll(".rabbit-hole__tooltip");
...
subscribeButton.addEventListener('click', function (e) {
	...
	if (!subscribeButton.classList.contains('_active')) {
		...
		subscribeButton.classList.remove('_active');
	}

});

const tests = document.querySelectorAll('.test');
const questions = document.querySelector('.test__content');
// const answerItem = document.querySelectorAll('.form-test__radiobutton');


tests.forEach((test) => {
	test.querySelectorAll('.form-test__input').forEach((item) => {
		item.addEventListener('change', function (e) {
			let currentButton = test.querySelector('.form-test__button');
			currentButton.removeAttribute('disabled');
			currentButton.addEventListener('click', function () {
				if (e.target.dataset.true === "1") {
					e.target.parentElement.classList.add('_correct');
					currentButton.innerHTML = "Правильно!";
					currentButton.classList.add('_correct');
					item.nextElementSibling.classList.add("_note");
					test.querySelectorAll('.form-test__input').forEach((item) => {
					...
					currentButton.innerHTML = "Не совсем так :(";
					currentButton.classList.add('_error');
					item.nextElementSibling.classList.add("_error");
					test.querySelectorAll('.form-test__input').forEach((item) => {
						if (item.dataset.true === "1") {
							item.parentElement.classList.add('_correct');
							item.nextElementSibling.classList.add("_note");
						}
						test.querySelectorAll('.form-test__input').forEach((item) => {
							item.setAttribute('disabled', 'disabled');
...
function mobileSlider() {
	if (window.innerWidth <= 600 && slider.dataset.mobile == "false") {
		document.querySelector('.slider-about__arrows').style.display = "flex";
		mySwiper = new Swiper(slider, {
			observer: true,
			// slideClass: 'slider-about__slide',
			observeParents: true,
			slidesPerView: 1,
			// loop: true,
			// spaceBetween: 10,
	...
	if (window.innerWidth > 600) {
		...
		document.querySelector('.slider-about__arrows').style.display = "none";
